...
export async function POST(request: NextRequest) {
  try {
    console.log('🤖 [Quiz AI Explain] Starting request...');
    
    const session = await getServerSession(authOptions);
    console.log('🤖 [Quiz AI Explain] Session:', session?.user?.id ? 'OK' : 'MISSING');
    ...
    console.log('🤖 [Quiz AI Explain] GROK_API_KEY:', GROK_API_KEY ? 'PRESENT' : 'MISSING');
    if (!GROK_API_KEY) {
      console.error('❌ [Quiz AI Explain] GROK_API_KEY not configured');
...
Ответь на РУССКОМ языке. Будь лаконичен (2-3 абзаца максимум).`;
    ...
    console.log('🤖 [Quiz AI Explain] Calling Grok API...');
    console.log('🤖 [Quiz AI Explain] Question:', questionText?.substring(0, 50) + '...');

    const grokUrl = 'https://api.x.ai/chat/completions';
    
    console.log('🤖 [Quiz AI Explain] Sending request to Grok...');
    ...
    console.log('🤖 [Quiz AI Explain] Response status:', response.status);
    
    if (!response.ok) {
      const error = await response.json();
      console.error('❌ [Quiz AI Explain] Grok API error:', error);
      return NextResponse.json(
    ...
    console.log('🤖 [Quiz AI Explain] Grok response received');
    console.log('🤖 [Quiz AI Explain] Full response:', JSON.stringify(data, null, 2));
    ...
    console.error('❌ Error in quiz ai-explain endpoint:', error);
    return NextResponse.json(
