...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!GROQ_API_KEY) {
      console.error('❌ [Quiz AI Explain] GROQ_API_KEY not configured');
      return NextResponse.json(
...
Ответь на РУССКОМ языке. Будь лаконичен (2-3 абзаца максимум).`;
    ...
    console.log('🤖 [Quiz AI Explain] Calling Groq API...');
    console.log('🤖 [Quiz AI Explain] Question:', questionText?.substring(0, 50) + '...');
    console.log('🤖 [Quiz AI Explain] API Key length:', GROQ_API_KEY?.length);

    // Вызываем Groq API
    console.log('🤖 [Quiz AI Explain] Sending request to Groq...');
    ...
    if (!response.ok) {
      ...
      console.error('❌ [Quiz AI Explain] Groq API error:', error);
      return NextResponse.json(
    ...
    console.log('🤖 [Quiz AI Explain] Groq response received');
