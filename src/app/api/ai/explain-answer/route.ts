...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🤖 [AI Explain Answer] GROQ_API_KEY:', GROQ_API_KEY ? 'PRESENT' : 'MISSING');
    if (!GROQ_API_KEY) {
      console.error('❌ [AI Explain Answer] GROQ_API_KEY not configured');
...
Ответь на РУССКОМ языке. Будь лаконичен (2-3 абзаца максимум).`;
    ...
    console.log('🤖 [AI Explain Answer] Calling Groq API...');
    console.log('🤖 [AI Explain Answer] Question:', questionText?.substring(0, 50) + '...');
    console.log('🤖 [AI Explain Answer] API Key length:', GROQ_API_KEY?.length);

    // Вызываем Groq API
    console.log('🤖 [AI Explain Answer] Sending request to Groq...');
    ...
    if (!response.ok) {
      ...
      console.error('❌ [AI Explain Answer] Groq API error:', error);
      return NextResponse.json(
    ...
    console.log('🤖 [AI Explain Answer] Groq response received');
